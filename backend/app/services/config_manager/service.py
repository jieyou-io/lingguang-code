...
class ConfigHubService:
    ...
    def read_provider_presets(self, engine: str) -> List[Dict[str, Any]]:
        ...
        对于 Gemini: 读取 ~/.lingguang/gemini_providers.json
        ...
        if engine_lower == "claude":
            ...
            presets_path = Path("~/.lingguang/gemini_providers.json").expanduser()
        else:
            return []
    ...
    def update_provider_config(self, engine: str, patch: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if engine_lower == "claude":
            ...
            # Claude 配置需要按 settings.json 结构更新
            env_patch = patch.pop("env", None)
            if env_patch:
            ...
            # 兼容旧参数：映射到 env
            if "apiKey" in patch and patch["apiKey"] is not None:
                env = data.get("env") if isinstance(data.get("env"), dict) else {}
                env["ANTHROPIC_API_KEY"] = patch.pop("apiKey")
                data["env"] = env
            if "baseUrl" in patch and patch["baseUrl"] is not None:
                env = data.get("env") if isinstance(data.get("env"), dict) else {}
                env["ANTHROPIC_BASE_URL"] = patch.pop("baseUrl")
                data["env"] = env
            if "model" in patch and patch["model"] is not None:
                env = data.get("env") if isinstance(data.get("env"), dict) else {}
                env["ANTHROPIC_MODEL"] = patch.pop("model")
                data["env"] = env
            if "temperature" in patch and patch["temperature"] is not None:
                env = data.get("env") if isinstance(data.get("env"), dict) else {}
                env["ANTHROPIC_TEMPERATURE"] = patch.pop("temperature")
                data["env"] = env
